      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <Card>
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-xs text-(--text-muted) uppercase">Transacciones</span>
          </div>
          <p className="text-2xl font-bold text-(--text-primary)">{filteredSales.length}</p>
        </Card>
        <Card>
          <div className="flex items-center gap-3 mb-2">
          ...
          <p className="text-2xl font-bold text-(--text-primary)">${avgOrder.toFixed(2)}</p>
        </Card>
        <Card>
          <div className="flex items-center gap-3 mb-2">
          ...
          <p className="text-2xl font-bold text-(--text-primary)">${cashTotal.toFixed(2)}</p>
        </Card>
        <Card>
          <div className="flex items-center gap-3 mb-2">
          ...
          <p className="text-2xl font-bold text-(--text-primary)">${cardTotal.toFixed(2)}</p>
        </Card>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-(--border-color)">
                <th className="text-left py-3 px-4 text-xs font-semibold text-(--text-muted) uppercase">ID</th>
                ...
                <th className="text-center py-3 px-4 text-xs font-semibold text-(--text-muted) uppercase">Método</th>
                <th className="text-center py-3 px-4 text-xs font-semibold text-(--text-muted) uppercase">Estado</th>
                <th className="text-right py-3 px-4 text-xs font-semibold text-(--text-muted) uppercase">Acción</th>
              </tr>
            </thead>
            <tbody>
                ...
                filteredSales.map((sale) => (
                  <tr key={sale.localId} className="border-b border-(--border-color)/50 hover:bg-(--bg-tertiary)">
                    <td className="py-4 px-4 text-right">
                      <span className="text-(--text-muted) font-mono text-xs">{sale.localId.slice(0, 8)}...</span>
                    </td>
            ...
            <div className="p-6 space-y-4">
              <div className="flex items-center justify-between pb-4 border-b border-(--border-color)">
                <div>
                  ...
                  <p className="text-(--text-primary) font-mono text-sm">{selectedSale.localId}</p>
                </div>
                    ...
                    <div key={idx} className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-(--text-secondary)">{item.quantity} x ${item.unitPrice.toFixed(2)}</p>
                        </div>
                      </div>
