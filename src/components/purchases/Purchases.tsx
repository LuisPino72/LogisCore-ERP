...
export default function Purchases() {
  ...
  const renderPurchaseCard = (purchase: Purchase) => {
    return (
      <div key={purchase.localId} className="bg-slate-900 border border-slate-800 rounded-xl p-4 shadow-lg hover:shadow-xl hover:border-slate-700 transition-all">
        <div className="flex items-start justify-between mb-3">
          <div>
            <h3 className="font-semibold text-white">{purchase.supplier}</h3>
            <p className="text-xs text-slate-500 font-mono">{purchase.invoiceNumber}</p>
          </div>
          ...
          <div className="mt-3 pt-3 border-t border-slate-800">
            {purchase.items.map((item, idx) => (
              <div key={idx} className="flex justify-between text-sm py-1">
                ...
                <span className="text-white">{item.quantity} x ${item.cost}</span>
              </div>
            ))}
          </div>
        )}
                    ...
                    {filteredPurchases.length === 0 ? (
                      ...
                      filteredPurchases.map((purchase) => (
                        <>
                          ...
                          {expandedPurchase === purchase.localId && (
                            <tr key={`${purchase.localId}-details`} className="bg-slate-950/50">
                              <td colSpan={6} className="py-4 px-4">
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                  {purchase.items.map((item, idx) => (
                                    <div key={idx} className="flex items-center justify-between bg-slate-800/50 p-2 rounded-lg">
                                      <span className="text-slate-300 text-sm">{item.productName}</span>
                                      <span className="text-white font-medium">{item.quantity} x ${item.cost}</span>
                                    </div>
                                  ))}
                                </div>
                              </td>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-800">
                    ...
                    <th className="text-left py-3 px-4 text-xs font-semibold text-slate-500 uppercase">Teléfono</th>
                    <th className="text-left py-3 px-4 text-xs font-semibold text-slate-500 uppercase">Notas</th>
                ...
                <tbody>
                  {filteredSuppliers.length === 0 ? (
                    ...
                    filteredSuppliers.map((supplier) => (
                      <tr key={supplier.localId} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                        ...
                        <td className="py-4 px-4 text-slate-400">{supplier.phone || '-'}</td>
                        <td className="py-4 px-4 text-slate-400 max-w-xs truncate">{supplier.notes || '-'}</td>
                        <td className="py-4 px-4 text-right">
                          <div className="flex items-center justify-end gap-1">
                            <button onClick={() => editSupplier(supplier)} className="p-2 hover:bg-slate-700 rounded-lg text-slate-400 hover:text-white transition-all">
                              <Edit2 className="w-4 h-4" />
                            </button>
                            <button onClick={() => handleDeleteSupplier(supplier.localId)} className="p-2 hover:bg-red-500/10 rounded-lg text-slate-400 hover:text-red-400 transition-all">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg shadow-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800">
              ...
              <button onClick={() => { setShowSupplierModal(false); setEditingSupplier(null); }} className="p-1 hover:bg-slate-700 rounded">
                <X className="w-5 h-5 text-slate-400" />
              </button>
            </div>
            <div className="p-6 space-y-4">
