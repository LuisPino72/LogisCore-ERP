...
import { 
  ShoppingBasket, Plus, Truck, Package, X, Check, Search, 
  LayoutGrid, List, Edit2, Trash2, User
} from 'lucide-react';
...
export default function Purchases() {
  ...
  const handleSubmit = async () => {
    ...
    setShowModal(false);
    setForm({ supplierId: undefined, supplierName: '', invoiceNumber: '', items: [] });
  ...
  const handleSupplierSubmit = async () => {
    ...
    if (editingSupplier) {
      ...
      await SyncEngine.addToQueue('suppliers', 'update', supplierData as unknown as Record<string, unknown>, supplierData.localId);
    } else {
      await db.suppliers.add(supplierData);
      await SyncEngine.addToQueue('suppliers', 'create', supplierData as unknown as Record<string, unknown>, supplierData.localId);
    }

    const updatedSuppliers = editingSupplier 
      ? suppliers.map(s => s.localId === editingSupplier.localId ? supplierData : s)
  ...
  const renderPurchaseCard = (purchase: Purchase) => {
    return (
      <div key={purchase.localId} className="bg-(--bg-secondary) border border-(--border-color) rounded-xl p-4 shadow-lg hover:shadow-xl transition-all">
        <div className="flex items-start justify-between mb-3">
          ...
          <span className={`px-2 py-1 rounded-full text-xs font-medium ${
            ...
            purchase.status === 'pending' ? 'bg-yellow-500/10 text-yellow-400' :
        ...
        <div className="pt-3 border-t border-(--border-color) flex items-center justify-between">
          ...
          <button 
            onClick={() => setExpandedPurchase(expandedPurchase === purchase.localId ? null : purchase.localId)}
          ...
          <div className="mt-3 pt-3 border-t border-(--border-color)">
            {purchase.items.map((item, idx) => (
              <div key={idx} className="flex justify-between text-sm py-1">
                ...
                <span className="text-(--text-primary)">{item.quantity} x ${item.cost}</span>
              </div>
            ))}
          </div>
        )}
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-(--text-secondary)">Gestión de proveedores y abastecimiento</p>
        </div>
        <div className="flex gap-2">
          ...
          {activeTab === 'suppliers' && (
            <Button onClick={() => { setEditingSupplier(null); setSupplierForm({ name: '', phone: '', notes: '' }); setShowSupplierModal(true); }}>
              <Plus className="w-4 h-4 mr-2" />
              Nuevo Proveedor
            </Button>
          )}
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card>
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-(--text-muted)">Total Completado</p>
            ...
            <Card>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-(--text-primary)">${(totalCompleted + totalPending).toFixed(2)}</p>
                </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-(--border-color)">
                      ...
                      <th className="text-left py-3 px-4 text-xs font-semibold text-(--text-muted) uppercase">Proveedor</th>
                      <th className="text-left py-3 px-4 text-xs font-semibold text-(--text-muted) uppercase">Factura</th>
                      <th className="text-center py-3 px-4 text-xs font-semibold text-(--text-muted) uppercase">Items</th>
                      <th className="text-right py-3 px-4 text-xs font-semibold text-(--text-muted) uppercase">Total</th>
                      <th className="text-center py-3 px-4 text-xs font-semibold text-(--text-muted) uppercase">Estado</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredPurchases.length === 0 ? (
                      ...
                      filteredPurchases.map((purchase) => (
                        <>
                          <tr 
                            ...
                            onClick={() => setExpandedPurchase(expandedPurchase === purchase.localId ? null : purchase.localId)}
                          >
                            <td className="py-3 px-4 text-(--text-primary)">{purchase.createdAt.toLocaleDateString()}</td>
                            <td className="py-3 px-4 text-(--text-secondary)">{purchase.supplier}</td>
                            <td className="py-3 px-4 text-(--text-muted) font-mono text-sm">{purchase.invoiceNumber}</td>
                            <td className="py-3 px-4 text-center text-(--text-secondary)">{purchase.items.length}</td>
                            <td className="py-3 px-4 text-right text-green-400 font-medium">${purchase.total.toFixed(2)}</td>
                            ...
                            <tr key={`${purchase.localId}-details`} className="bg-(--bg-primary)/50">
                              <td colSpan={6} className="py-4 px-4">
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                  {purchase.items.map((item, idx) => (
                                    <div key={idx} className="flex items-center justify-between bg-(--bg-tertiary)/50 p-2 rounded-lg">
                                      <span className="text-(--text-secondary) text-sm">{item.productName}</span>
                                      <span className="text-(--text-primary) font-medium">{item.quantity} x ${item.cost}</span>
                                    </div>
                                  ))}
                                </div>
                              </td>
                ...
                <tbody>
                  {filteredSuppliers.length === 0 ? (
                    ...
                    filteredSuppliers.map((supplier) => (
                      <tr key={supplier.localId} className="border-b border-(--border-color) hover:bg-(--brand-500)/5">
                        ...
                        <td className="py-4 px-4 text-(--text-muted) max-w-xs truncate">{supplier.notes || '-'}</td>
                        <td className="py-4 px-4 text-right">
                          <div className="flex items-center justify-end gap-1">
                            <button onClick={() => editSupplier(supplier)} className="p-2 hover:bg-(--bg-tertiary) rounded-lg text-(--text-muted) hover:text-(--text-primary) transition-all">
                              <Edit2 className="w-4 h-4" />
                            </button>
                            <button onClick={() => handleDeleteSupplier(supplier.localId)} className="p-2 hover:bg-red-500/10 rounded-lg text-(--text-muted) hover:text-red-400 transition-all">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-(--bg-primary) border border-(--border-color) rounded-2xl w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-6 py-4 border-b border-(--border-color) sticky top-0 bg-(--bg-primary)">
              ...
              <button onClick={() => setShowModal(false)} className="p-1 hover:bg-(--bg-tertiary) rounded">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {form.items.map((item, index) => (
                    <div key={index} className="flex items-center justify-between bg-(--bg-tertiary) p-3 rounded-lg border border-(--border-color)">
                      <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => removeItem(index)} className="p-1 hover:bg-(--bg-primary) rounded">
              ...
              <Button className="w-full" onClick={handleSubmit} disabled={form.items.length === 0}>
                Registrar Compra
              </Button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-(--bg-secondary) border border-(--border-color) rounded-2xl w-full max-w-lg shadow-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-(--border-color)">
              ...
              <button onClick={() => { setShowSupplierModal(false); setEditingSupplier(null); }} className="p-1 hover:bg-(--bg-tertiary) rounded">
                <X className="w-5 h-5 text-(--text-muted)" />
              </button>
            </div>
            <div className="p-6 space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-(--border-color)">
                <Button variant="secondary" onClick={() => { setShowSupplierModal(false); setEditingSupplier(null); }}>
                  Cancelar
                </Button>
                <Button onClick={handleSupplierSubmit} disabled={!supplierForm.name}>
