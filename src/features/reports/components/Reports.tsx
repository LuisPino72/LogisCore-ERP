...
import { getReportsData, DateRange, Stats, TopProduct } from "../services/reports.service";
import { Sale } from "@/lib/db";
import Card from "@/common/Card";
import { TrendingUp, DollarSign, ShoppingCart, Package, BarChart3, ShoppingBag, CreditCard, Banknote, PackageX } from "lucide-react";

export default function Reports() {
  const [dateRange, setDateRange] = useState<DateRange>("month");
  const [stats, setStats] = useState<Stats>({ totalSales: 0, totalPurchases: 0, totalProducts: 0, totalOrders: 0, avgOrderValue: 0, cashPayments: 0, cardPayments: 0 });
  ...
  useEffect(() => {
    ...
    getReportsData(tenant.slug, dateRange).then(data => {
      setStats(data.stats);
  ...
  const kpis = [
    ...
    { label: "Ganancia Neta", value: `$${(stats.totalSales - stats.totalPurchases).toFixed(2)}`, icon: TrendingUp, color: stats.totalSales - stats.totalPurchases >= 0 ? "green" : "red" },
  ];
        ...
        <div className="flex items-center gap-2 bg-(--bg-secondary) border border-(--border-color) rounded-xl p-1">
          {(["today", "week", "month", "all"] as DateRange[]).map((range) => (
            <button key={range} onClick={() => setDateRange(range)}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${dateRange === range ? "bg-(--brand-600) text-white" : "text-(--text-secondary) hover:text-(--text-primary) hover:bg-(--bg-tertiary)"}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            <div>
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2"><div className="w-10 h-10 bg-green-500/10 rounded-lg flex items-center justify-center"><Banknote className="w-5 h-5 text-green-400" /></div><span className="text-(--text-secondary)">Efectivo</span></div>
                <span className="text-(--text-primary) font-bold">${stats.cashPayments.toFixed(2)}</span>
              </div>
              <div className="h-3 bg-(--bg-tertiary) rounded-full overflow-hidden">
                <div className="h-full bg-green-500 rounded-full transition-all" style={{ width: getPaymentWidth(stats.cashPayments) }} />
              </div>
            </div>
            <div>
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2"><div className="w-10 h-10 bg-(--brand-500)/10 rounded-lg flex items-center justify-center"><CreditCard className="w-5 h-5 text-(--brand-400)" /></div><span className="text-(--text-secondary)">Tarjeta</span></div>
                <span className="text-(--text-primary) font-bold">${stats.cardPayments.toFixed(2)}</span>
              </div>
              <div className="h-3 bg-(--bg-tertiary) rounded-full overflow-hidden">
                <div className="h-full bg-(--brand-500) rounded-full transition-all" style={{ width: getPaymentWidth(stats.cardPayments) }} />
              </div>
            </div>
          </div>
        </Card>
        <Card>
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between"><div className="flex items-center gap-2"><PackageX className="w-4 h-4 text-red-400" /><span className="text-(--text-secondary)">Sin Stock</span></div><span className="text-red-400 font-bold">0</span></div>
                  ...
                  recentSales.map((sale) => (
                    <tr key={sale.localId} className="border-b border-(--border-color)/50 hover:bg-(--bg-tertiary)">
                      ...
                      <td className="py-3 px-2 text-right">
                        <span className={`px-2 py-0.5 rounded text-xs ${sale.paymentMethod === "cash" ? "bg-green-500/10 text-green-400 border border-green-500/20" : "bg-(--brand-500)/10 text-(--brand-400) border border-(--brand-500)/20"}`}>
