            ...
            <div className="space-y-3">
              ...
              <div className="pt-3 border-t border-(--border-color)">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span
                    className={`font-bold ${canInvite ? "text-green-400" : "text-amber-400"}`}>
                ...
                <div className="h-2 bg-(--bg-tertiary) rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.min((currentUsedSlots / maxEmployees) * 100, 100)}%`,
                    }}
                  />
                </div>
              </div>
            ...
            {!canInvite && (
              <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-3">
                <p className="text-xs text-amber-400">
                  Has alcanzado el límite máximo de empleados.
                </p>
              </div>
            )}
          </div>
                  ...
                  <label className="flex items-center gap-2.5 text-sm text-(--text-primary) cursor-pointer">
                    <input
                      ...
                      onChange={(e) =>
                        setPerms({
                          ...perms,
                          can_create_inventory: e.target.checked,
                        })
                      }
                  ...
                  filteredInvitations.map((inv) => (
                    <tr
                      ...
                      <td className="py-4 px-4">
                        <div className="flex flex-wrap gap-1.5">
                          {Object.entries(
                            (inv.permissions as Record<string, any>) || {},
                          ).map(
                            ([key, val]) =>
                              val === true && (
                                <span
                                  key={key}
                                  className="px-2 py-0.5 bg-amber-500/10 text-amber-400 text-[10px] rounded-md border border-amber-500/20 font-medium">
                                  {key.replace("can_", "")}
                                </span>
                              ),
                          )}
